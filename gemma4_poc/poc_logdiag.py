"""Gemma4-12B agentic PoC — 로그 읽기→이상구간 특정→진단 실행→판정 (LEO 지시).

판정 대상: 통신사 판이 아니라 **우리가 실제로 시키는 일**로 쓸 만한가.
시나리오는 2026-08-02에 oven이 실제로 수행한 추론 사슬을 그대로 재현한다:
  로그 파싱률 이상 관측 → 구간 특정 → LLM 서버 설정 조회 → 근본원인(슬롯 컨텍스트) 판정

도구 결과는 전부 **실제 실측치**를 넣는다(가공 수치 아님).
사람 확인 없는 배치 환경을 시스템 지시로 명시 → 자율 연쇄 여부까지 같이 본다.
"""
import json
import sys
import time
...
def call(messages, label, max_tokens=700):
    req = {"model": "gemma4", "messages": messages, "tools": TOOLS, "tool_choice": "auto",
           "max_tokens": max_tokens, "temperature": 1.0, "repeat_penalty": 1.1}
    t0 = time.time()
