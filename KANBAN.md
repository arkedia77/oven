...
- [x] **하모니시티 라이브 시뮬 운용 — 재가동+rep_floor** — oven/ogo — 2026-07-19 재기동, 관찰중
  ...
  - ✅✅ **07-23 D-A2 원인규명 완료 — 형식이탈46%는 모델한계 아닌 공학적 문제(토큰예산)**: 3변형 비교(N=20 each) — baseline(mt512)=45%, final-marker+추론허용(mt512)=**0%**(역설적 악화), baseline(mt1536, 토큰만 상향)=**100%**. "thinking 없이" 억제문구가 실제 유효함을 반증. `autonomy.py` max_tokens 512→1536 수정+재검증 10/10=100% 확인. **완주율 게이트(모델한계시 확대금지) 재판정 필요 — 이미 해소됐으므로 D-A3 재개 여부 kee 판단 대기**
