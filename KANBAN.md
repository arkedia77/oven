...
② **다음 세션 할 일**: 🔴⓪**kee 판정 도착·이행 착수(A-089 `[[WAIT:oven]]`)** — 순서 고정: **①watchdog 재배선 → ②FAIL 표본 실증 → ③temp 0.3**(③은 ①②가 선행조건)
    - **①watchdog**: **②sim.log 최종 갱신 시각 = 주 판정**(«진행»을 잼) + **①run_village 명령줄 매칭 = 보조 병행**(배타 아님, R-P4). ★kee 논거: «개별 헬스체크를 늘려온 게 처방이 아니었다 — 판정 축 자체가 「존재」인 것이 원인. 하나 더 만들면 다음엔 또 다른 방향으로 같은 갭». **①만으로는 halt로 멈춘 프로세스가 통과**함
    - ★**임계 N분은 손으로 고르지 말 것** — **tick 간격 실측의 배수**(예: p95 × k)로 산출하고 **k의 근거를 문서에 적을 것**(임의값 하드코딩 금지)
    - ★★**②FAIL 표본 필수(R-P5)**: 새 watchdog 켠 뒤 **sim.log를 인위로 정지시켜 실제 재기동되는지 1회 실증**하고 **«N분 뒤 재기동됨» 로그를 kee 통지에 실을 것**. 안 하면 «도달 불가인 채 매번 PASS»가 됨
    - **③temp 0.3 승인·조건 3**: ⑴하모니시티 안정 확인 후 ⑵**PoC 스레드 10→4** ⑶라벨을 **«샘플러 민감도 측정»**(«제작자 주장 재현» 아님), 두 판을 **같은 표 다른 행 + 각 행에 조건 병기**
    - ★**판정 경계(확대 금지)**: temp 0.3 통과 = «샘플러로 갈린다»**까지**, 실패 = «이 시나리오에선 샘플러로 안 갈린다»**까지**. **어느 쪽도 «쓸 만하다/못 쓴다»로 확대 금지**(표본 1판)
    - kee 잔여 요청: **503 원인이 사후 규명되면 통지**(미확정으로 닫지 않고 열어 둠)
    - ✅ kimsecretary에 **판정문 공유 완료**(요청⑥). 13h50m 공백 별건은 **kee가 A-089로 이미 채번**해 중복 발신 불요 확정
    - 📌 **kimsecretary 보고 시점 = «temp 실측 후»로 확정**(2안 중 택1). 사유: 3안(GPU확보/배치한정/보류)의 **선택지 구성 자체가 temp 결과에 따라 바뀌므로** 반쪽으로 올려 고르게 하지 않음. ※격하 사실 자체는 08-04 LEO께 구두 보고됨 — **미보고 방치가 아니라 «3안 상신»만 지연**
    - 📌 **oven 의무 1건(상시)**: 조건이 바뀌거나 **일정이 더 밀리면 그 사실만 kimsecretary에 통지**할 것(그쪽 보고 시점이 여기 걸려 있음). kimsecretary 잔여는 «oven 산출 3건 대기» 하나뿐 — ⓐ**watchdog 판정 방식 변경 승인 여부**(프로세스 개수 → run_village 명령줄 매칭 or sim.log 갱신시각. 이번 13h50m 미인지의 직접 원인) ⓑ**Gemma4 PoC를 temp 0.3으로 재측정할지**(제작자 «이슈 99%는 샘플러 설정» 고지 때문. 돌린다면 PoC 스레드 10→4로 줄이거나 하모니시티 안정 확인 후) ①**하모니시티 안정 관찰** — 08-03 킬스위치 정발동 이력 있음, 조기경보 창(400 이벤트)은 재기동으로 리셋돼 다시 채우는 중 ②**회귀 조기경보 첫 낮 틱 확인** — `prompt=N자` 필드가 라이브 로그에 실제로 찍히는지(재기동은 심야에 해서 아직 미출력). `HarmonicityParseWatch`(6h 주기)가 `prompt_chars_median`을 채우기 시작하면 정상 ①**3일 관찰 리포트(~08-03)는 4구간 비교로 확정** — ①고장 ②슬롯4096·mt1536 ③mt2560(87.62%) ④A단계(14004~), 구간별 관계지표 분리집계 + **①②구간에 「fallback=갈등 미기록」 캐비엇 필수** + `zero_relations`는 rep_floor 스펙 v0의 침식경로 한정 판독 규칙과 함께 인용 **<95%면 kee 재회부 없이 ②A단계 즉시 집행**(ctx 16384/parallel 2 + mt 3072, 사전승인 실물=`oven_kee_20260731_001500_...`), ≥95%면 현행 동결. 서버 미기동 시 즉시 롤백·보고 ①**하모니시티 3일 관찰(kee 지정, ~08-03)** — 전환 tick 13564 기준 전/후로 ①파싱 성공률 ②관계지표(rep_floor·포화율) 분리 보고(자연 A/B) — decision_records 적재율/틱당 오버헤드·KILL_SWITCH 오발동 0·스냅샷 회전·rep_floor 0.15 효과. 3일 후 kee 회신 필수 ②거동변경 게이트 3종(ECONOMY/INSTITUTION/AUTONOMY_LOCATION)은 **LEO 판단 대기** — 임의 활성화 금지 ③hf-playground/Leo 새 지시 대기(큐 비어있음). ari 커밋관례(author=oven, `-c user.name/email`) 계속 적용.
