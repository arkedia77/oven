...
- [ ] **연구 파일럿 1호 — 구조문법주입 도메인전이(데이터효율 한계곡선)** — oven/kee — 2026-07-16 착수 게이트 진행중
  ...
  - 🟢 **19:34 진행상황**: frac50 epoch1 step3000/3268(102.7min 경과, ~2.05s/step) — epoch1 완료 임박(~19:11 예상), epoch2 이어서, 그 다음 frac70
...
- [ ] **Krea2 이미지 캠페인 (hf-playground 협업)** — oven/ogo/hf-playground — 🟢 현재 큐 비어있음
  ...
  - 🟡 **19:34 진행상황**: 4/12 완료, 5번째(street_film_plain_seed42) 25/32step(~203s/step 확정) — 곧 완료 예상. street_film Raw 이미지는 장당 ~1.8h(1024대비 4배) — 남은 Raw street_film 3장(~5.4h)+edit 4장(해상도별 상이) 감안 시 전체 완주까지 상당 시간(수시간 추가) 소요 예상
