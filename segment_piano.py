"""Segment long piano wavs into 30s clips, filter silence."""
import os, sys, random, glob
from pathlib import Path
import numpy as np
import soundfile as sf

RAW_DIR = r"D:\data\piano-v2\raw"
OUT_DIR = r"D:\data\piano-v2\segments"
SEG_SEC = 30.0
SR_TARGET = 48000
RMS_MIN = 0.01           # skip near-silent
PEAK_MAX = 0.99          # avoid clipping
MAX_PER_SONG = 20        # cap per source
STRIDE_SEC = 45.0        # slide window for candidates
RANDOM_SEED = 42
...
for wav_path in sorted(glob.glob(os.path.join(RAW_DIR, "*.wav"))):
    ...
    for start in candidates:
        ...
        if len(out_name) > 180:
            out_name = f"{name[:60]}_seg{start//sr:05d}.wav"
        out_path = os.path.join(OUT_DIR, out_name)
        sf.write(out_path, seg, sr, subtype="PCM_16")
        kept_this += 1
        kept_total += 1
    print(f"[OK] {name}: kept {kept_this} / {len(candidates)} candidates", flush=True)
