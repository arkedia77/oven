...
from village.memory.core import add_key_event
...
def check_events(
    ...
    for event in events:
        ...
        char_id = event.get("character")
        if char_id:
            add_key_event(char_id, f"[Day {world.day}] {event['description']}")
        elif event.get("characters"):
            for cid in event["characters"]:
                add_key_event(cid, f"[Day {world.day}] {event['description']}")
...
def _check_stagnation(relationships: dict, world: WorldState) -> list[dict]:
    ...
    if avg_tension >= 0.2:
        world.low_tension_days = 0
        return []
    world.low_tension_days += 1
    if world.low_tension_days >= 3:
        world.low_tension_days = 0
        return [{
