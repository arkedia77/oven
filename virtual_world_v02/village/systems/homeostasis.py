...
def decay_relationships(relationships: dict):
    """매 틱 자연 감쇠 (P2: warmth/trust, P2.2: affection 자연 감쇠 추가)."""
    for rel in relationships.values():
        ...
        if affection > 0.5:
            a_excess = affection - 0.5
            affection = max(0.5, affection - a_excess * 0.004)
        # 0.9 soft cap: 초과분은 30%만 인정 (특별 이벤트로 일시 초과는 허용하되 고착 방지)
        if affection > AFFECTION_SOFT_CAP:
            affection = AFFECTION_SOFT_CAP + (affection - AFFECTION_SOFT_CAP) * 0.3
...
def apply_soft_ceiling(rel: dict):
    ...
    # P2.2: baseline(0.5) 초과분만 실링 완화에 반영.
    # (기존 affection*0.2는 affection 0.5에서 이미 0.95 포화 → 감쇠가 무의미했음)
    # affection 1.0→실링 0.95, 0.9→0.93, 0.7→0.89, 0.5이하→0.85
    relax = max(0.0, affection - 0.5) * 0.2
    warmth_ceil = min(0.95, WARMTH_SOFT_CEILING + relax)
    trust_ceil = min(0.95, TRUST_SOFT_CEILING + relax)
