"""Quincy P2 eval — generate samples with tempo prefix (slow/medium/fast/very_fast)."""
...
LORA_DIR = r"D:\liszt\output\quincy_p2\lora_checkpoints\lora_best"
EVAL_DIR = r"C:\Users\leo\liszt\eval\quincy_p2_eval"
...
def analyze_midi(midi_path):
    ...
    if len(starts) > 4:
        ...
        if diffs:
            avg_ioi = sum(diffs) / len(diffs) / 1e6  # seconds
...
if __name__ == "__main__":
    ...
    # P2 Eval: tempo × form/genre combinations
    ...
    # 1) Tempo × Form (pop genre) — 16 combinations
    for tempo in tempos:
        for form in forms:
            styles.append({
    ...
    # 2) Tempo × Genre (ballad form) — 8 combinations
    for tempo in tempos:
        for genre in genres:
            name = f"{tempo}_ballad_{genre}"
    ...
    # 3) Tempo-only (no form/genre) — 4 combinations
    for tempo in tempos:
        styles.append({
            "name": f"{tempo}_only",
            "prompt": [
    ...
    print(f"\nTotal styles to generate: {len(styles)}", flush=True)
    ...
    for style in styles:
        ...
        for idx, seq in enumerate(results):
            ...
            print(f"  sample_{idx}: notes={a['notes']} uniq={a['unique_pitches']} "
                  f"rep={a['rep_8gram']}% vel={a['vel_range']} dur={a['duration_s']}s "
                  f"bpm≈{a['est_bpm']}", flush=True)
    ...
    # Summary table
    print(f"\n{'='*80}", flush=True)
    print(f"{'Style':<30} {'Notes':>6} {'Uniq':>5} {'Rep%':>6} {'Vel':>5} "
          f"{'VStd':>5} {'Dur':>6} {'BPM':>5}", flush=True)
    print("-" * 80, flush=True)
    for name, data in all_results.items():
        ...
        if n == 0:
            continue
        print(f"{name:<30} "
              f"{sum(s['notes'] for s in ss)/n:>6.0f} "
              f"{sum(s['unique_pitches'] for s in ss)/n:>5.1f} "
              f"{sum(s['rep_8gram'] for s in ss)/n:>5.1f}% "
              f"{sum(s['vel_range'] for s in ss)/n:>5.0f} "
              f"{sum(s['vel_std'] for s in ss)/n:>5.1f} "
              f"{sum(s['duration_s'] for s in ss)/n:>5.1f}s "
              f"{sum(s['est_bpm'] for s in ss)/n:>5.0f}", flush=True)

    print(f"\nResults: {rpath}", flush=True)
    print(f"MIDI files: {EVAL_DIR}", flush=True)
